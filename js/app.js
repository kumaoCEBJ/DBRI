        ...
        myWorker.onmessage = function (e) {
            ...
            switch (message.type) {
                ...
                case 'preresult':
                    ...
                    break;
                case 'result':
                    if (dotNetHelper && message.data) {
                        const byteArray = new Uint8Array(message.data);
                        dotNetHelper.invokeMethodAsync('HandleWorkerResult', byteArray, byteArray.length);
                    }
                    break;
                case 'torehan':
                    if (dotNetHelper && message.data) { 
                        const byteArray = new Uint8Array(message.data);
                        dotNetHelper.invokeMethodAsync('HandleWorkerResultTorehan', byteArray, byteArray.length);
                    }
                    break;
                case 'error':
                    if (dotNetHelper && message.payload) {
...
window.cancelWorkerProcessing = function () {
    if (myWorker) {
        ...
        if (dotNetHelper) {
            dotNetHelper.invokeMethodAsync('HandleWorkerCancelled', 'Canceled by user');
        }
    }
};
